...
Not verifying the SSH host key is not an option for security reasons. If you don't know the expected SSH host key, please find it out in a secure manner. For a newly provisionned virtual machine, you may want to take a peek at the X-Cli/ssh2vsock provider to perform a secure ssh-keyscan.
- `password` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) password specifies the password used to authenticate to the SFTP server.
...
- `private_key` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) private_key specifies the private key to use to authenticate to the SFTP server.

The private key may be encrypted. If so, the private_key_passphrase attribute must be specified.

Exactly one of the password, private_key, private_key_path or use_agent attributes must be specified.
- `private_key_passphrase` (String, Sensitive, [Write-only](https://developer.hashicorp.com/terraform/language/resources/ephemeral#write-only-arguments)) private_key_passphrase specifies the passphrase to use to decrypt the private key specified with the private_key or private_key_path attributes.
